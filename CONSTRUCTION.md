...
MemoryTreeVLA 是一个基于层级记忆树（Hierarchical Memory Tree, HMT）的视觉-语言-动作模型（VLA），
针对长时程机器人操作任务设计。其核心思想是：

- 在线构建 HMT，将连续轨迹组织为跨时间尺度的树形记忆结构
- 用 JumpAwareHead 检测动作突变点（= 分支点），自动分割子任务边界
- 用 SGMTS（Semantic-Guided Mamba Tree-Scan）在树结构上进行语义引导的视觉特征提取
...
```
  ...
  └──┬───────────────────┬────┘              └──────────┬───────────┘
     ...
     ▼                   │                              ▼
  ┌──────────────────────────────────┐    ┌──────────────────────────────────────┐
  │               SGMTS               │◄───┤     HierarchicalMemoryTree (HMT)     │
  │         Mamba 视觉树扫描           │s_top│  Merge: Welford 更新 z_v, a_hist    │
  │   引导: g_task + s_top (← HMT)   │    │  Branch+Elevate:                     │
  └──────────────┬────────────────────┘    │    兄弟叶子 MLPElevation → s_abs      │
                 │ Z_v (B, N_v, d)          └──────────────▲───────────┬────────────┘
...
**文件**: `memory_tree_vla/model/semantic_jump_head.py`
...
### 3.2 SGMTS（语义引导 Mamba 树扫描编码器）

**文件**: `memory_tree_vla/model/sgmts/sgmts.py`

SGMTS 在 GrootV 的 Mamba 树扫描（`tree_scanning.py: MinimumSpanningTree + BFS + Tree_SSM`）基础上引入层级语义引导，将任务描述和 HMT 高层抽象节点的语义同时注入扫描过程。
...
#### 步骤 2：语义根节点选择（区别于 GrootV 的固定左上角根）

GrootV 中 BFS 根隐式由 MST 结构决定，等效于从某固定角落出发。SGMTS 改为**显式选择语义根**：

$$r^* = \arg\max_{i \in \{1,\ldots,N_p\}} \cos(p_i,\, g_\text{sem})$$

其中 $p_i \in \mathbb{R}^d$ 为 patch $i$ 的视觉特征，$r^*$ 是当前帧中与 $g_\text{sem}$ 最相关的 patch，作为 BFS 的起始根。Mamba SSM 沿以 $r^*$ 为根的 BFS 序列扫描，确保最先被「记忆」的是语义最相关区域。

---

#### 步骤 3：语义加权 MST 构建

相邻 patch 的边权（取代 GrootV 纯 Cosine）：

$$w_{ij} = \underbrace{\cos(p_i,\, p_j)}_{\text{视觉相似度}} + \alpha\;\underbrace{\cos(p_i,\, g_\text{sem})\cdot\cos(p_j,\, g_\text{sem})}_{\text{任务/子任务相关度偏置}}$$

$\alpha = 0.5$（默认）。与 GrootV 一致，用 `MinimumSpanningTree("Cosine", torch.exp)` 在新边权上建 MST，再从 $r^*$ 出发 BFS 得扫描序列。

> **效果**：高权边连接两个都与当前子任务相关的 patch，MST 优先将它们串联；BFS 从语义中心向外展开，SSM 隐态在遍历到非相关区域前已编码充分的任务上下文。

---

#### 步骤 4：Tree-SSM 前向（沿 BFS 序进行选择性扫描）

同 GrootV `tree_scanning_core`，SSM 的 $\Delta_t$ 由视觉特征决定（选择性机制），递推方式为：

$$h_i = \bar{A}_i \odot h_{\text{parent}(i)} + \bar{B}_i \odot x_i, \qquad z_i = C_i^\top h_i + D \odot x_i$$

其中父子关系由上一步 BFS 树决定（非线性链，树形依赖）。最终每个 patch 的输出 $z_i$ 携带了从根到该 patch完整路径上的上下文信息。

输出：全局池化后得到帧级视觉表示：

$$z_v = \text{MeanPool}(\{z_i\}) \in \mathbb{R}^{d_\text{visual}}$$
...
|------|-------------|--------------|
| BFS 根 | 固定（隐式左上角） | 动态语义根 $r^* = \arg\max \cos(p_i, g_\text{sem})$ |
| 边权 | 纯视觉 Cosine | 视觉 Cosine + 任务/子任务相关度偏置 |
| 语义引导来源 | 无 | 总任务嵌入 + HMT 顶层抽象节点（随树成长自适应切换） |
| 语义层级 | 单层（无记忆） | 双层：$g_\text{task}$（全局）+ $\bar{s}_\text{top}$（当前子任务） |

**三条简单但有效的创新**：
1. **语义根漂移**（Semantic Root Drift）：每帧根据当前 $g_\text{sem}$ 重选 $r^*$，根随任务焦点自然漂移，无任何额外参数。
2. **子任务偏置边**（Subtask-biased MST）：Branch+Elevate 产生新抽象节点 $s_k$ 后，立即以 $g_\text{sem} \leftarrow s_k$ 刷新边权，MST 自动重组以聚焦新子任务区域。
3. **$\beta$ 调度**（Task-to-Subtask Schedule）：$\beta$ 从 1.0 线性衰减到 0.3，树越深越信任子任务语义而非总任务，无需任何监督信号。

### 3.3 HierarchicalMemoryTree（层级记忆树）

**文件**: `memory_tree_vla/model/memory_tree/`
...
| 情况 | 触发条件 | 描述 |
|------|---------|------|
| **Case A**（最低满足） | 第一个抽象节点即 $d_\text{first} \geq \tau_\text{mount}$ | s_cur 与该子任务差异已足够大，不需插入中间层 |
| **Case B**（中间满足） | $d_\text{first} < \tau_\text{mount}$，爬升中某节点 $v_k$ 满足 $d_k \geq \tau_\text{mount}$ | s_cur 代表更高粒度的语义，需新建中间抽象层 |
| **Case C**（超出根节点） | 爬升至根仍无 $d_k \geq \tau_\text{mount}$ | s_cur 语义超出了整棵树，需创建新的顶层节点 |

> **核心直觉**：
> - **Case A**："第一抽象层就能区分" → 说明新帧在当前子任务下只是另一个普通分支，挂在已有抽象节点下即可，**不需要新建中间抽象层**（避免频繁提升）
> - **Case B**："需要爬升才能区分" → 说明新帧属于更高层次的语义切换，需要在 $v_k$ 下**新建一个语义探针抽象节点**来表示这个新子任务分支
> - **Case C**："超出整棵树" → 说明新帧的语义高于现有所有层级，需要**创建超级根**来容纳它
...
| 参数 | 默认值 | 含义 |
|------|-------|------|
| `mount_tau` | 0.4 | 余弦距离阈值；大 → 更难触发 Case A → 树倾向 Case B/C（更深）；小 → 树更扁平 |
...
**文件**: `memory_tree_vla/model/fusion.py`
...
**文件**: `memory_tree_vla/model/action_head/flow_matching.py`
...
```
        预训练阶段（两步）                     LIBERO                LIBERO
   （详见下方阶段说明）              →    Phase 1         →    Phase 2
─────────────────────────────────────     ─────────────         ──────────────
[a] mini-imagenet                         冻结: LLM +           冻结: 无
  · 仅 SGMTS 可训练                             全部预训练模块
  · 损失: L_cls（视觉分类交叉熵）
                                          可训练: Fusion,       可训练: 全部
[b] RoboCerebra                                   FlowHead              (LLM: 0.1× LR)
  · 冻结: LLM, Fusion, FlowHead
  · 可训练: SGMTS, s_proj,                损失: L_flow          损失: L_flow
    JumpAwareHead,                            （仅此一项）            （仅此一项）
    TreeSSM, MLPElevation
  · 损失: L_boundary
    + L_sem + L_elev
...
#### [a] SGMTS 骨干预训练（mini-imagenet）

目标：为 SGMTS 视觉骨干建立判别性特征提取能力，使后续全模型预训练从有意义的视觉表征出发（类比 GrootV 在 ImageNet 上预训练视觉骨干）。

- SGMTS（含 Tree-SSM、MST 模块）在 mini-imagenet 分类任务上端到端训练，损失为交叉熵 $\mathcal{L}_\text{cls}$
- 其余模块（LLM、Fusion、FlowHead、JumpAwareHead、HMT 相关层）全部冻结
- 训练完成后，SGMTS 权重作为全模型预训练（RoboCerebra）的初始化，不再使用 mini-imagenet 数据

---

#### [b] 全模型预训练（RoboCerebra）

目标：让模型学会"什么是子任务边界"以及"边界处的语义应当与子任务描述对齐"。

- `JumpAwareHead` 学习从动作序列中识别动作突变点（纯动作信号，不依赖语义）
- `SGMTS` 学习在树形结构上进行语义引导的视觉扫描
...
```
SGMTS 预训练（mini-imagenet） →  L_cls（视觉分类交叉熵）
...
**文件**: `memory_tree_vla/losses/tree_losses.py`
...
│
├── scripts/
│   ├── pretrain.py              ← 预训练脚本（SGMTS mini-imagenet + RoboCerebra 全模型）
...
├── memory_tree_vla/
│   ├── model/
│   │   ├── memory_tree_vla.py   ← 主模型（MemoryTreeVLA, 三阶段 forward）
...
└── dataset/
    ...
    │   ├── libero_object/
    │   └── libero_spatial/
    └── mini-imagenet/           ← SGMTS 视觉骨干预训练数据集（分类任务）
```
